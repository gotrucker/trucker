...
// 	changesets := make([]*db.Changes, 0, 8)
...
begin / startstream
- if transaction struct = nil criar transaction struct
  - return transaction
- else panic

relationmessagev2
- currentTable = nome da tabela (esta mensagem não é repetida entre trasações se a tabela não mudar)
- meter nome da tabela na transaction struct

insert / update / delete message
- if changes struct = nil then
  - criar changes struct
  - set table
  - set operation
  - set columns
- append to rows in changes struct

StreamStop
- put changes struct in transaction struct changes channel
- reset changes struct to nil

commitmessage / stream commit message
- put changes struct in transaction struct changes channel
- close transaction changes channel
- reset changes struct to nil
- reset transaction struct to nil
- set inStream = false
